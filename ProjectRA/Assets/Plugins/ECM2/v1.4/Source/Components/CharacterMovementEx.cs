...
namespace ECM2
{
    public class CharacterMovementEx : CharacterMovement
    {
        #region ENUMS
        ...
        [SerializeField]
        private float _rotationRate = 540;
        
        [Space(15f)]
        [Tooltip("The Character's default movement mode. Used at player startup.")]
        [SerializeField]
        private MovementMode _startingMovementMode = MovementMode.Walking;
        ...
        [SerializeField]
        private float _maxAcceleration = 10;
        ...
        [SerializeField]
        private float _brakingDecelerationWalking = 20;
        ...
        [SerializeField]
        private bool _canEverJump = true;

        [Tooltip("Can jump while crouching ?")]
        [SerializeField]
        private bool _canJumpWhileCrouching = true;
        ...
        [SerializeField]
        private float _flyingFriction = 1;
        ...
        [SerializeField]
        private float _standingDownwardForceScale = 1;
        ...
        protected virtual void UpdatePhysicsVolume(PhysicsVolume newPhysicsVolume)
        {
            ...
            Vector3 characterCenter = worldCenter;
        ...
        public override void RotateTowards(Vector3 worldDirection, float deltaTime, bool updateYawOnly = true)
        {
            ...
            rotation = Quaternion.RotateTowards(rotation, targetRotation, rotationRate * deltaTime);
        }
        ...
        protected virtual void RotateWithRootMotion()
        {
            if (useRootMotion && rootMotionController)
                rotation = rootMotionController.ConsumeRootMotionRotation() * rotation;
        }
        ...
        protected virtual void ConsumeRotationInput()
        {
            ...
            if (_rotationInput != Vector3.zero)
            {
                ...
                rotation *= Quaternion.Euler(_rotationInput);
        ...
        public virtual float GetMaxBrakingDeceleration()
        {
            switch (_movementMode)
            {
                ...
                case MovementMode.Falling:
                {
                    ...
                    return isOnGround ? 0.0f : brakingDecelerationFalling;
                }
        ...
        /// </summary>
        /// ref : CharacterMovement.ApplyVelocityBraking
        ...
        /// </summary>
        /// ref : CharacterMovement.CalcVelocity
        ...
        public virtual Vector3 ConstrainInputVector(Vector3 inputVector)
        {
            ...
            return ConstrainVectorToPlane(inputVector);
        }
        ...
        protected virtual void CalcDesiredVelocity(float deltaTime)
        {
            ...
            // Vector3 desiredVelocity = useRootMotion && rootMotionController? rootMotionController.ConsumeRootMotionVelocity(deltaTime) : movementDirection * GetMaxSpeed();
            Vector3 desiredVelocity = useRootMotion && rootMotionController ? rootMotionController.ConsumeRootMotionVelocity(deltaTime) : movementDirection;
        ...
        public float GetSignedSlopeAngle()
        {
            ...
            Vector3 projMovementDirection =
                Vector3.ProjectOnPlane(movementDirection, groundNormal).normalized;
        ...
        protected virtual void WalkingMovementMode(float deltaTime)
        {
            ...
            else
            {
                ...
                velocity = CalcVelocity(velocity, GetDesiredVelocity(), groundFriction, false, deltaTime);
        ...
        {
            bool overlapped = CheckHeight(_unCrouchedHeight);
            return !overlapped;
        }
        ...
        protected virtual bool DoJump()
        {
            ...
            if (isConstrainedToPlane && 
                Mathf.Approximately(Vector3.Dot(GetPlaneConstraintNormal(), worldUp), 1.0f))
            {
                return false;
            }
            ...
            float verticalSpeed = Mathf.Max(Vector3.Dot(velocity, worldUp), jumpImpulse);

            velocity = Vector3.ProjectOnPlane(velocity, worldUp) + worldUp * verticalSpeed;
        ...
        protected virtual void FlyingMovementMode(float deltaTime)
        {
            ...
            else
            {
                ...
                velocity = CalcVelocity(velocity, GetDesiredVelocity(), actualFriction, true, deltaTime);
            }
        }
        ...
        protected virtual void UpdateRotation(float deltaTime)
        {
            ...
            else if (_rotationMode == RotationMode.OrientRotationToViewDirection && cameraTransform != null)
            {
                // Determines if rotation should modify character's yaw only
        ...
        public void SetRootMotionController(RootMotionController rootMotionController)
        {
            _rootMotionController = rootMotionController;
        }
        ...
        protected override void Reset()
        {
            ...
            _gravity = new Vector3(0.0f, -15.0f, 0.0f);
            _gravityScale = 1.0f;
            ...
            collisionLayers = ~0; //LayerMask.GetMask("Character", "Player", "Shield");
        }
